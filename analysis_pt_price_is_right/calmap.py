...
Modified to allow text annotations on the calendar.
"""
...
def yearplot(data, year=None, how='sum', vmin=None, vmax=None, cmap='Reds',
    ...
    if text is None:
        text = pd.Series('', index=data.index)
    ...
    if how is None:
        ...
        if _pandas_18:
            ...
            text = text.resample('D').agg(how)
        else:
            by_day = data.resample('D', how=how)
            text = text.resample('D', how=how)
    ...
    text_data = by_day.pivot('day', 'week', 'text').values[::-1]
    text_data = np.ma.masked_where(text_data != 'R', text_data)
    for i in range(text_data.shape[0]):
            for j in range(text_data.shape[1]):
                if text_data[i, j] == 'R':
                    text = ax.text(j + .5, i + .5, text_data[i, j],
